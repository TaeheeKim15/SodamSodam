...
public interface StoreMapper {
	@Select("select s.sno, sname, u.uno, sint, stel, saddr, s_det_addr, dyesno, sphoto, edt, sdt, scdt, scategory "
			+ "from tmk_store_info s " + "join tmk_user u on s.uno = u.uno" + " order by" + " s.sno asc")
	List<Store> findStoreList();

	@Select("select s.sno, sname, u.uno, sint, stel, saddr, s_det_addr, dyesno, sphoto, edt, sdt, scdt, scategory "
			+ "from tmk_store_info s " + "join tmk_user u on s.uno = u.uno" + " where s.uno = #{uno}" + " order by" + " s.sno asc")
	Store findByUnoStore(int uno);
	...
	@Select("SELECT s.sno, p.pname FROM tmk_store_info s " + "JOIN tmk_product p " + "ON s.sno = p.sno "
			+ "WHERE s.sno =#{sno}")
	...
	@Select("SELECT rno, rcontent, rscore, cdt " + "FROM tmk_review tr " + "JOIN tmk_store_info ts ON tr.sno = ts.sno "
			+ "JOIN tmk_user tu ON tr.uno = tu.uno " + "WHERE tr.sno = #{sno} AND tr.uno = #{uno}")
